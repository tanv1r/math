...
\subsection*{Solution}
Say the perimeter is $a$. If one side of the rectangle is $x$, then the other side must be $\frac{a}{2} - x$. Now the are is a polynomial:  $x\ \left( \frac{a}{2}-x \right) = -x^2 + x\cdot \frac{a}{2} = -\left( x - \frac{a}{4} \right)^2 + \frac{a^2}{16}$. Thus the product has the maximum value when $x = \frac{a}{4}$. But in that case the other side is also $\frac{a}{4}$, making it a square.
\end{document}
