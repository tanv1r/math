...
\begin{itemize}
	...
	\begin{equation*}
		...
		\end{aligned}
	\end{equation*}
	We now use \textbf{proof by contradiction} to show that $a^{\delta} > 1$ thus $a^p < a^{p+\delta}$. Say, $1 > a^{\delta}$. Since any root of 1 is 1 itself, if we now take $\delta$-th root on both side, we have $1 > a$; a contradiction. \contradiction

Note the tacit assumption here that taking positive root of two positive numbers does not change the magnitude-relation between them.
