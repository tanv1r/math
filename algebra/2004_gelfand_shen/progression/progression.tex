...
\subsection*{Solution}
Say the second term is $x$. Then from the definition of a geometric progression, $\frac{x}{a} = \frac{b}{x}$. Therefore, $x^2 = ab$. If $ab < 0$, there is no real-valued $x$; in other words, there is no such real-valued geometric progression. For example, even if the ratio were negative like -1, the geometric progression would alternate the signs, so that first and third terms would still have the same sign. If $ab = 0$, then $x = 0$. If $ab > 0$, there are two possibilities for $x$: $\sqrt{ab}$ and $-\sqrt{ab}$.
