...
\newthought{Let} $C(x)$ be the point we need. Therefore $d(x, -9) = \frac{1}{2}d(x, -3)$. We note that on the number axis, $B(-3)$ is to the right of $A(-9)$; so, $C(x)$ cannot be to the right of $B(-3)$ because then we would have $d(x, -9) > d(x, -3)$. Hence $C(x)$ can possibly be between $A(-9)$ and $B(-3)$ or to the left of $A(-9)$. We consider these two cases below:
\begin{enumerate}
	\item On the number axis, from left to right, the order of the three points is: $A(-9), C(x), B(-3)$. We have the below two equations:
	\begin{equation*}
		\begin{aligned}
			d(x, -9) + d(x, -3) &= d(-9, -3)\\
...
$x=-7$ is the unique solution to both equations; thus $C(-7)$ is the only valid choice between $A(-9)$ and $B(-3)$.
	\item On the number axis, from left to right, the order of the three points is: $C(x), A(-9), B(-3)$. We have the below two equations:
	\begin{equation*}
		\begin{aligned}
			d(x, -3) - d(x, -9) &= d(-9, -3)\\
			d(x, -9)            &= \frac{1}{2}d(x, -3)
