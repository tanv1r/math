...
\newthought{Let} $C(x)$ be the center. We can find $x$ by adding half of the distance between $A(x_1)$ and $B(x_2)$, therefore $\frac{d(A, B)}{2}$ to the smaller of $x_1$ and $x_2$.
\[
x = \begin{cases} x_2 + \frac{d(A, B)}{2} &\mbox{if } x_1-x_2 \geq 0 \\
x_1 + \frac{d(A, B)}{2} & \mbox{if } x_1-x_2 < 0 \end{cases}
\]
We can consolidate the above cases as follows:
\begin{equation*}
	\begin{aligned}
