...
\title{Notes from \\The Method of \\Coordinates} % Title of the book

\author{Mohammad Tanviruzzaman} % Author
...
A number axis is a line with three important pieces of information associated with it:
\begin{enumerate}
	\item An origin,
...
\newthought{Solve} the equation $|x+1| + |x+2| = 1$.

\newthought{Let} $x+1 = a$. Then $x+2 = a+1$. So, the given equation becomes
\[
	|a| + |a+1| = 1
\]
...
\newthought{On a number axis} two points $A(x_1)$ and $B(x_2)$ are given. Find the coordinate of the center of the segment $AB$.

\newthought{Let} $C(x)$ be the center. We shall consider the six possible cases separately and then try to generalize.
\begin{enumerate}
	\item $A(x_1)$ is to the left of $B(x_2)$. Here we have three different sub-cases. 
\break
For sub-case \footnote{\begin{tikzpicture}[scale=0.5]
\def\labelz{{"Origin","$A(x_1)$","","$B(x_2)$"}}
\draw [->,>=triangle 45] (0,0) -- (10,0);
\foreach \x in {1,...,4} {%
        \draw [-,>=stealth] (2*\x,-.2) -- (2*\x,.2);
        \node [align=center,text width=22mm] at (2*\x,0.5){\pgfmathparse{\labelz[\x-1]}\pgfmathresult};
}
\end{tikzpicture}}, 
\[d(A, B) = d(B, Origin) - d(A, Origin) = (x_2-0) - (x_1-0) = x_2 - x_1\]
For sub-case \footnote{\begin{tikzpicture}[scale=0.5]
\def\labelz{{"$A(x_1)$","Origin","","$B(x_2)$"}}
\draw [->,>=triangle 45] (0,0) -- (10,0);
\foreach \x in {1,...,4} {%
        \draw [-,>=stealth] (2*\x,-.2) -- (2*\x,.2);
        \node [align=center,text width=22mm] at (2*\x,0.5){\pgfmathparse{\labelz[\x-1]}\pgfmathresult};
...
\end{tikzpicture}}, 
\[
	d(A, B) = d(B, Origin) + d(A, Origin) = (x_2-0)+(0-x_1) = x_2-x_1
\]
For sub-case \footnote{\begin{tikzpicture}[scale=0.5]
\def\labelz{{"$A(x_1)$","","$B(x_2)$","Origin"}}
\draw [->,>=triangle 45] (0,0) -- (10,0);
\foreach \x in {1,...,4} {%
        \draw [-,>=stealth] (2*\x,-.2) -- (2*\x,.2);
        \node [align=center,text width=22mm] at (2*\x,0.5){\pgfmathparse{\labelz[\x-1]}\pgfmathresult};
...
\end{tikzpicture}},
\[
	d(A, B) = d(A, Origin) - d(B, Origin) = (0-x_1) - (0-x_2) = x_2-x_1
\]
Hence, for the sub-cases, $2$, $3$, and $4$ we have:
\[
	x = x_1 + \frac{1}{2}d(A, B) = x_1 + \frac{x_2 - x_1}{2} = \frac{x_1 + x_2}{2}
\]
\item $B(x_2)$ is to the left of $A(x_1)$. Again, we have three sub-cases.
\break
For sub-case \footnote{\begin{tikzpicture}[scale=0.5]
\def\labelz{{"Origin","$B(x_2)$","","$A(x_1)$"}}
\draw [->,>=triangle 45] (0,0) -- (10,0);
\foreach \x in {1,...,4} {%
        \draw [-,>=stealth] (2*\x,-.2) -- (2*\x,.2);
        \node [align=center,text width=22mm] at (2*\x,0.5){\pgfmathparse{\labelz[\x-1]}\pgfmathresult};
...
\end{tikzpicture}}
\[
	d(A, B) = d(A, Origin) - d(B, Origin) = (x_1 - 0) + (x_2-0) = x_1-x_2
\]
For sub-case \footnote{\begin{tikzpicture}[scale=0.5]
\def\labelz{{"$B(x_2)$","Origin","","$A(x_1)$"}}
\draw [->,>=triangle 45] (0,0) -- (10,0);
\foreach \x in {1,...,4} {%
        \draw [-,>=stealth] (2*\x,-.2) -- (2*\x,.2);
        \node [align=center,text width=22mm] at (2*\x,0.5){\pgfmathparse{\labelz[\x-1]}\pgfmathresult};
...
\end{tikzpicture}}
\[
	d(A, B) = d(B, Origin)+d(A, Origin) = (0-x_2)+(x_1-0) = x_1-x_2
\]
For sub-case 
\footnote{\begin{tikzpicture}[scale=0.5]
\def\labelz{{"$B(x_2)$","","$A(x_1)$","Origin"}}
\draw [->,>=triangle 45] (0,0) -- (10,0);
\foreach \x in {1,...,4} {%
        \draw [-,>=stealth] (2*\x,-.2) -- (2*\x,.2);
        \node [align=center,text width=22mm] at (2*\x,0.5){\pgfmathparse{\labelz[\x-1]}\pgfmathresult};
...
\[
	d(A, B) = d(B, Origin) - d(A, Origin) = (0-x_2)-(0-x_1) = x_1-x_2
\]
Hence, for the sub-cases $5$, $6$, and $7$, we have:
\[
	x = x_2 + \frac{1}{2}d(A, B) = x_2 + \frac{x_1 - x_2}{2} = \frac{x_1 + x_2}{2}
\]
\end{enumerate}
\break
So, in all cases, the coordinate of the center is $\frac{x_1 + x_2}{2}$.
...
\newthought{Find} the coordinate of the point on the number axis which is located twice as close to the point $A(-9)$ as to the point $B(-3)$.

\newthought{Let} $C(x)$ be the point we need. Therefore $d(x, -9) = \frac{1}{2}d(x, -3)$. We note that on the number axis, $B(-3)$ is to the right of $A(-9)$\footnote{\begin{tikzpicture}[scale=0.5]
\def\labelz{{"$A(-9)$", "", "$B(-3)$", "", "Origin"}}
\draw [->,>=triangle 45] (0,0) -- (12,0);
\foreach \x in {1,...,5} {%
        %\draw [-,>=stealth] (2*\x,-.1) -- (2*\x,.1);
        \node [align=center,text width=22mm] at (2*\x,0.5){\pgfmathparse{\labelz[\x-1]}\pgfmathresult};
}
\draw [-,>=stealth] (2*1,-.1) -- (2*1,.2);
\draw [-,>=stealth] (2*3,-.1) -- (2*3,.2);
\draw [-,>=stealth] (2*5,-.1) -- (2*5,.2);
\end{tikzpicture}}; so, $C(x)$ cannot be to the right of $B(-3)$ because then we would have $d(x, -9) > d(x, -3)$. Hence $C(x)$ can possibly be between $A(-9)$ and $B(-3)$ or to the left of $A(-9)$. We consider these two cases below:
\begin{enumerate}
	\item On the number axis, from left to right, the order of the three points is: $A(-9), C(x), B(-3)$\footnote{\begin{tikzpicture}[scale=0.5]
\def\labelz{{"$A(-9)$", "$C(x)$", "", "$B(-3)$", "Origin"}}
\draw [->,>=triangle 45] (0,0) -- (12,0);
\foreach \x in {1,...,5} {%
        %\draw [-,>=stealth] (2*\x,-.1) -- (2*\x,.1);
        \node [align=center,text width=22mm] at (2*\x,0.5){\pgfmathparse{\labelz[\x-1]}\pgfmathresult};
...
\draw [-,>=stealth] (2*4,-.1) -- (2*4,.2);
\draw [-,>=stealth] (2*5,-.1) -- (2*5,.2);
\end{tikzpicture}}. We have the below two equations:
	\begin{equation*}
		\begin{aligned}
			d(x, -9) + d(x, -3) &= d(-9, -3)\\
			d(x, -9)            &= \frac{1}{2}d(x, -3)
		\end{aligned}
	\end{equation*}
$x=-7$ is the unique solution to both equations; thus $C(-7)$ is the only valid choice between $A(-9)$ and $B(-3)$.
	\item On the number axis, from left to right, the order of the three points is: $C(x), A(-9), B(-3)$\footnote{\begin{tikzpicture}[scale=0.5]
\def\labelz{{"$C(x)$", "$A(-9)$", "", "$B(-3)$", "Origin"}}
\draw [->,>=triangle 45] (0,0) -- (12,0);
\foreach \x in {1,...,5} {%
        %\draw [-,>=stealth] (2*\x,-.1) -- (2*\x,.1);
        \node [align=center,text width=22mm] at (2*\x,0.5){\pgfmathparse{\labelz[\x-1]}\pgfmathresult};
...
\end{tikzpicture}}. We have the below two equations:
	\begin{equation*}
		...
		\end{aligned}
	\end{equation*}
$x = -15$ is the unique solution to both equations, so $C(-15)$ is the only valid choice to the left of $A(-9)$.
\end{enumerate}
