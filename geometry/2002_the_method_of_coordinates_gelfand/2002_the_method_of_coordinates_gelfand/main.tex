...
\newthought{Let} $C(x)$ be the center. We can find $x$ by adding half of the distance between $A(x_1)$ and $B(x_2)$ aka $\frac{1}{2}d(A, B)$, to the smaller of $x_1$ and $x_2$.
\[
x = 
\begin{cases} 
	x_2 + \frac{1}{2}d(A, B) &\mbox{if } x_1-x_2 \geq 0 \\
	x_1 + \frac{1}{2}d(A, B) &\mbox{if } x_1-x_2 < 0 
\end{cases}
\]
Since $C(x)$, the center, is equally distant from both $A(x_1)$ and $B(x_2)$, we can consolidate the above cases as follows:
\begin{equation*}
	\begin{aligned}
		x &= x_1 + \frac{1}{2}(x_2 - x_1)\\
		  &= \frac{x_1 + x_2}{2}
	\end{aligned}
\end{equation*}
...
$x=-7$ is a solution to both equations; thus $C(-7)$ is a valid choice.
	...
	\begin{equation*}
		...
		\end{aligned}
	\end{equation*}
$x = -15$ solves both equations, so $C(-12)$ is another valid choice.
\end{enumerate}
